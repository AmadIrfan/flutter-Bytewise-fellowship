...
class _create_playerState extends State<create_player> {
  ...
  Future<void> downloadURLfunc(cuserid, player_name) async {
    String imgurl = await firebase_storage.FirebaseStorage.instance
        .ref('images/player_photos/$currentuserid/$__team_name/$player_name.png')
        .getDownloadURL();
    setState(() {
      downloadURL = imgurl;
    });
  ...
  Future _query() async {
    return playerargs[0]['isedit']
        ...
        : players.doc(currentuserid).collection(__teamname[0]['team_name']).add({
            'player_name': _playername.text,
            'player_role': player_role.toString(),
            'batting_style': batting_style.toString(),
            'bowling_style': bowling_style.toString(),
  ...
  Future<void> savedata() async {
    return _query().then((value) {
      ...
      playerargs[0]['isedit'] ? customtoast('Player Updated') : customtoast('Player Added');
    }).catchError((error) {
      setState(() {
        inprogress = false;
      });
      playerargs[0]['isedit'] ? customtoast("Failed to update Player: $error") : customtoast("Failed to add Player: $error");
